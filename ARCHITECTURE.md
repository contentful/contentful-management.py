...
**Upstream (consumes):**
- Contentful Management API (`api.contentful.com` or `api.eu.contentful.com`) — all read/write operations
- Contentful Upload API (`upload.contentful.com` or `upload.eu.contentful.com`) — binary asset uploads
...
| `_docs/` | Sphinx documentation source (auto-generated into `docs/`). Do not hand-edit `docs/` — it is re-generated by `pdm run docs`. The one exception is `docs/ADRs/`, which holds hand-authored decision records and is excluded from the build's prune. |
